...
type Heartbeat struct {
	...
	VirtType      string                 `protobuf:"bytes,19,opt,name=virt_type,json=virtType,proto3" json:"virt_type,omitempty"`                 // 虚拟化类型 (podman / cloudhv)
...
// CmdGetPortForwards 的响应，携带 command_id 与请求关联
...
type PlatformEnvelope_GetPortFwds struct {
	GetPortFwds *CmdGetPortForwards `protobuf:"bytes,14,opt,name=get_port_fwds,json=getPortFwds,proto3,oneof"` // 查询某 VM 的端口转发规则列表
}
...
// 查询某 VM 的端口转发规则列表（结果通过 CommandResult.data JSON 回传）
...
const file_proto_agent_agent_proto_rawDesc = "" +
	"\n" +
	"\x17proto/agent/agent.proto\x12\x05agent\"\x96\x02\n" +
	...
	"\tvirt_type\x18\x13 \x01(\tR\bvirtType\x12\"\n" +
	...
	"delPortFwd\x125\n" +
	"\vget_vm_info\x18\n" +
	" \x01(\v2\x13.agent.CmdGetVMInfoH\x00R\tgetVmInfo\x121\n" +
...
var file_proto_agent_agent_proto_depIdxs = []int32{
	...
	6,  // 3: agent.AgentEnvelope.port_fwd_list:type_name -> agent.PortForwardList
	...
	7,  // 7: agent.PortForwardList.entries:type_name -> agent.PortForwardEntry
	1,  // 8: agent.PortForwardEntry.protocol:type_name -> agent.Protocol
	...
	12, // 19: agent.PlatformEnvelope.update_vm:type_name -> agent.CmdUpdateVM
	11, // 20: agent.PlatformEnvelope.reinstall_vm:type_name -> agent.CmdReinstallVM
	...
	2,  // 24: agent.AgentGateway.Connect:input_type -> agent.AgentEnvelope
	10, // 25: agent.AgentGateway.Connect:output_type -> agent.PlatformEnvelope
